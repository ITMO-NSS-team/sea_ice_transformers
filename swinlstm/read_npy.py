    ...
    def __getitem__(self, idx):
        ...
        if self.pad:
            ...
            return np.pad(self.data[:, idx:idx + self.period],
                          ...
                          'constant',
                          constant_values=0), self.data[:,
                                              idx + self.period:idx + self.period + self.predict_period], self.dates[
                                                                                                          idx:idx + self.period], self.dates[
                                                                                                                                  idx + self.period:idx + self.period + self.predict_period]
        else:
            return self.data[:, idx * self.shift:idx * self.shift + self.period], self.data[:,
                                                                                  idx * self.shift + self.period:idx * self.shift + self.period + self.predict_period], self.dates[
                                                                                                                                                                        idx * self.shift:idx * self.shift + self.period], self.dates[
                                                                                                                                                                                                                          idx * self.shift + self.period:idx * self.shift + self.period + self.predict_period]
...
def choose_datetime_period(dir_path, from_ymd: Union[List[int], None], to_ymd: Union[List[int], None]) -> List[str]:
    ...
    if from_ymd is not None:  # Choose dates that equal or later than from_ymd.
        list_dir = [file for file in list_dir if datetime(*[datetime.strptime(file, f'{name}_%Y%m%d.npy').year,
                                                            ...
                                                            datetime.strptime(file,
                                                                              f'{name}_%Y%m%d.npy').day]) >= datetime(
            *from_ymd)]
...
List[torch.utils.data.DataLoader]], tuple]:
    ...
    if train_test_split is not None:
        ...
        if len(list_dir[train_part::stride]) // (predict_period + in_period) == 0 or len(
                list_dir[:train_part:stride]) // (predict_period + in_period) == 0:
            print(
        ...
        while len(list_dir[train_part::stride]) // (predict_period + in_period) == 0 or len(
                list_dir[:train_part:stride]) // (predict_period + in_period) == 0:
