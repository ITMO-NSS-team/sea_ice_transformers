...
name = '104_52_results'
path_to_train = fr'D:\Projects\test_cond\AAAI_code\Ice/kara'
...
for i in range(start,range_,step):
    img = resize(np.load(path_to_dir+'/'+dir_list[i]), (140, 140), anti_aliasing=False)
    img = np.fliplr(img)
    img = np.rot90(img, 2)
...
plt.savefig(
            f'images/{range_}_predict_{name}_step_{step}.png', dpi=300)
plt.show()
...
for i in range(start,range_,step):
    ...
    plt.title(f'{dates[i].year}-{dates[i].month}-{dates[i].day}', fontsize=21)
plt.savefig(
            f'images/{range_}_GT_{name}_step_{step}.png', dpi=300)
plt.show()