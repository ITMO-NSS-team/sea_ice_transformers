...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0.1, act_layer=nn.ReLU, norm_layer=nn.LayerNorm, attention_type='divided_space_time'):
        super().__init__()
...
class PatchEmbedAugmeted(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.patch_size = patch_size
        self.num_patches = num_patches
        self.conv2_2 = nn.Sequential(nn.Conv2d(in_channels=in_chans,out_channels=in_chans*4,kernel_size=3,stride=(1,1),padding=(1,1)),
                                        nn.ReLU(),
                                        nn.BatchNorm2d(in_chans*4),
                                        nn.Conv2d(in_channels=in_chans*4,out_channels=in_chans*4,kernel_size=3,stride=(1,1),padding=(1,1)),
                                        nn.ReLU(),
                                        nn.BatchNorm2d(in_chans*4))
        self.proj = nn.Conv2d(in_chans*(4+1), embed_dim, kernel_size=patch_size, stride=patch_size)
...
class TimeSformerIntoAugmented(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        ...
        self.pretrained=False
        #self.emb_dim=kwargs['embeded_dim']
        self.model = VisionTransformerAugmented(img_size=img_size, num_classes=num_classes, patch_size=patch_size, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., num_frames=num_frames, attention_type=attention_type, **kwargs)
