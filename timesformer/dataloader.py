...
class IceDataset(Dataset):
    ...
    def __len__(self):
        """
        Returns the length of the time series data based on shift, period and predict_period.

          Args:
            None

          Returns:
            int: The number of samples in the processed time series data.
        """
        self.lent = [
            i for i in range(self.data.shape[1] - self.period - self.predict_period + 1)
        ]
        return int(
            (self.data.shape[1] - self.period + 1) / self.shift
        )  # -self.predict_period
    ...
    def __getitem__(self, idx):
        ...
        if self.pad:
            self.pad_shape = (
                self.data[:, idx * self.shift : idx * self.shift + self.period].shape[
                    0
                ],
                self.data[:, idx * self.shift : idx * self.shift + self.period].shape[
                    1
                ...
                np.max(
                    self.data[
                        :, idx * self.shift : idx * self.shift + self.period
                    ].shape
                ),
                np.max(
                    self.data[
            ...
            return (
                np.pad(
                    self.data[:, idx : idx + self.period],
                    [
                        (0, i)
                        for i in np.subtract(
                            self.pad_shape, self.data[:, idx : idx + self.period].shape
                        )
                    ],
                    "constant",
                    constant_values=0,
                ...
                self.data[
                    :, idx + self.period : idx + self.period + self.predict_period
                ],
                self.dates[idx : idx + self.period],
                self.dates[idx + self.period : idx + self.period + self.predict_period],
            )
        else:
            return (
                self.data[:, idx * self.shift : idx * self.shift + self.period],
                self.data[
                    :,
                    idx * self.shift
                    + self.period : idx * self.shift
                    + self.period
                    + self.predict_period,
                ],
                self.dates[idx * self.shift : idx * self.shift + self.period],
                self.dates[
                    idx * self.shift
                    + self.period : idx * self.shift
                    + self.period
    ...
    def __load_npz__(self):
        """
        Loads data from .npz files in the specified directory.
            ...
            Returns:
                None
        """
        l_dir = self.list_dir[:: -self.stride]
        l_dir.reverse()
        ...
        for i, file in enumerate(l_dir[:: self.stride]):
            if i == 0:
                ...
                x = np.float32(
                    np.load(self.path_to_dir + f"/{file}", allow_pickle=True)
                )
                ...
                if self.resize is not None:
                    data = resize(
                        data, (self.resize[0], self.resize[1]), anti_aliasing=False
                    )
                ...
                self.data = np.concatenate(
                    (self.data, data), axis=1
                )  # Concattanete by frame dimision


def choose_datetime_period(
    dir_path, from_ymd: Union[List[int], None], to_ymd: Union[List[int], None]
) -> List[str]:
    ...
    if from_ymd is not None:  # Choose dates that equal or later than from_ymd.
        list_dir = [
            ...
            if datetime(
                *[
                    datetime.strptime(file, f"{name}_%Y%m%d.npy").year,
                    datetime.strptime(file, f"{name}_%Y%m%d.npy").month,
                    datetime.strptime(file, f"{name}_%Y%m%d.npy").day,
                ]
            )
            >= datetime(*from_ymd)
        ]
...
def clip_data_by_batch_and_period(
    data_list: List[str], batch_size: int, period: int, predict_period: int
):
    """
    Clips the input data list to a length determined by batch size and periods.

        Calculates a clipping length based on the input data's length, batch size,
        and specified period/prediction period, then returns the clipped list.

        Args:
            data_list: The list of data strings to clip.
            batch_size: The desired batch size for processing.
            period:  The primary time period.
            predict_period: The prediction time period.

        Returns:
            List[str]: A clipped version of the input data list.
    """
...
def create_dataloaders(
    ...
    Union[torch.utils.data.DataLoader, List[torch.utils.data.DataLoader]], tuple
]:
    """
    ...
    """
    list_dir = choose_datetime_period(
        dir_path=path_to_dir, from_ymd=from_ymd, to_ymd=to_ymd
    )
    if train_test_split is not None:
        ...
        if (
            len(list_dir[train_part::stride]) // (predict_period + in_period) == 0
            or len(list_dir[:train_part:stride]) // (predict_period + in_period) == 0
        ):
            print(
                "You choose too short date period! You may choose bigger period between from_ymd and to_ymd. Or choose lower periods for predicts"
            )
        ...
        while (
            ...
            new_f = datetime(*from_ymd) - timedelta(
                days=stride * (predict_period + in_period) // 2
            )
            new_to = datetime(*to_ymd) + timedelta(
                days=stride * (predict_period + in_period) // 2
            )
        ...
        train_data = clip_data_by_batch_and_period(
            data_list=train_data,
            batch_size=batch_size,
            period=in_period,
            predict_period=predict_period,
        )
        test_data = clip_data_by_batch_and_period(
            data_list=test_data,
            batch_size=batch_size,
            period=in_period,
            predict_period=predict_period,
